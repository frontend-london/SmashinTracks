...
$date = date('Y-m-d', time() - $day); // -day bo wysyla po polnocy
...
//$week_start = '2011-12-13';
    ...
    $sql = "SELECT *  FROM `transactions` WHERE `transactions_date` LIKE '$date%' AND `transactions_done` = '1'";
    ...
    $sql3 = "SELECT `profiles_id`, `profiles_name`, `profiles_email` FROM `profiles` WHERE profiles_id IN ($artists_ids_text) AND profiles_sales_inform_daily = 1"; 
    ...
    while($w3 = mysql_fetch_array($result3)) {
        ...
        $content_text = "Hello $profiles_name,\nYou have sold following tracks last day:\n\n";
        $counter = 0;
        ...
        $content_html = '<html>
        <head>
        <title>Your Tracks sold on Smashintracks.com last day:</title>
        </head>
        <body>';
        ...
        $subject = 'Your Tracks sold on Smashintracks.com last day';

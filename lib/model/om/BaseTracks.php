...
abstract class BaseTracks extends BaseObject  implements Persistent {
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Tracks has previously been saved, it will retrieve
	 * related TracksVotess from storage. If this Tracks is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collTracksVotess === null) {
			if ($this->isNew()) {
			   $this->collTracksVotess = array();
				...
				$criteria->add(TracksVotesPeer::TRACKS_ID, $this->tracks_id);
	 ...
	 * actually need in Tracks.
	 */
	public function getTracksVotessJoinProfiles($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(TracksPeer::DATABASE_NAME);
		}
		...
		if ($this->collTracksVotess === null) {
			if ($this->isNew()) {
				...
				$this->collTracksVotess = TracksVotesPeer::doSelectJoinProfiles($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
